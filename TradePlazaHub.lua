...
==================================================================================
    ...
    Deplacement : par defaut le personnage se deplace avec de la VRAIE physique
    (aucune ecriture de CFrame), ce que les anticheats de position ne voient pas
    comme un teleport. Vitesse reglable, protection anti-chute a l'arrivee.
...
local CONFIG = {
    ...
    -- deplacement
    TPMode       = "physique",  -- physique | marche | cframe
    TPSpeed      = 45,          -- studs/seconde (45 = a peine plus qu'une course)
    TPAltitude   = 18,          -- hauteur de survol
    TPNoclip     = false,       -- traverser les murs (plus visible pour un anticheat)
...
----------------------------------------------------------------------------------
-- DEPLACEMENT
--   mode "physique" : on n'ecrit JAMAIS de CFrame. On pose un BodyVelocity sur le
--   personnage et on le laisse se deplacer avec le moteur physique, exactement
--   comme un joueur qui court ou tombe. Pour le serveur c'est un mouvement
--   continu et legitime : rien a detecter comme teleport, et pas de degat de
--   chute puisque la vitesse de descente reste controlee.
----------------------------------------------------------------------------------
local TP = {}
...
-- ancien mode : ecriture directe de CFrame, le plus rapide mais le plus visible
function TP.stepTo(goalCF)
    local speed, guard = Util.clamp(CONFIG.TPSpeed, 10, 250), 0
    setNoclip(true)
    while true do
        if State.Unloaded or State.Cancel then setNoclip(false) return false, "annule" end
        local root = TP.root()
        if not root then setNoclip(false) return false, "personnage perdu" end
        local delta = goalCF.Position - root.Position
        local dist = delta.Magnitude
        if dist < 3 then setNoclip(false) return true end
        local dt = RunService.Heartbeat:Wait()
        guard = guard + dt
        if guard > 75 then setNoclip(false) return false, "trajet trop long" end
...
function TP.travel(goalCF)
    ...
    elseif CONFIG.TPMode == "cframe" then ok, err = TP.stepTo(goalCF)
...
-- tous les remotes qui peuvent servir a inviter, le plus probable en premier
...
function Trade.invite(query)
    ...
    --    (capture automatique en arriere-plan)
    local template = Spy and Spy.templates and Spy.templates.invite
    if template then
        local ok = Spy.replay(template, { target = player, userId = userId })
        if ok then return true, "trade envoye a " .. who end
    end

    -- 2) sinon : chaque remote candidat x chaque forme d'argument
    local candidates, tried = Trade.candidates(), {}
    for index, remote in ipairs(candidates) do
        ...
        for _, shape in ipairs(argShapes(player, userId)) do
            ...
            if not skip and shape.args[1] ~= nil then
                ...
                table.insert(tried, remote.Name .. ":" .. shape.label .. "=" .. status)
                log("%s[%s] -> %s (%s)", remote.Name, shape.label, status, tostring(res))
                if status == "ok" and res ~= false then
                    CONFIG.ArgMode = shape.label
                    return true, "trade envoye a " .. who .. " (" .. remote.Name .. ")"
                end
            end
            -- un RemoteEvent ne repond jamais : on ne le spamme pas
            if not isFunction then break end
        end
        if index >= 3 then break end
    end

    if #tried == 0 then
        return false, "aucun remote d'invitation trouve dans ce jeu"
    end
    return false, "le serveur a refuse (" .. tostring(#tried) .. " essais) - ouvre le vrai menu de trade une fois, le hub apprendra l'appel"
end
...
local cardMove = card(pageSettings, "Deplacement",
    "physique = mouvement reel, aucun CFrame ecrit : c'est le mode le moins reperable")
chips(cardMove, { "physique", "marche", "cframe" }, "TPMode")
...
switch(cardMove, "Traverser les murs (plus reperable)", "TPNoclip")
switch(cardMove, "Atterrissage freine (anti degats de chute)", "TPLandSlow")
note(cardMove, "Tu meurs ou tu es renvoye en arriere ? Reste en mode physique et descends la vitesse a 25-45. Laisse 'traverser les murs' desactive : c'est ce que la plupart des anticheats reperent en premier.", THEME.warn)
...
GENV.TradePlazaHub = {
    ...
    Spy = Spy, Hook = Hook, Unload = unload,
}
